...
`GET /api/source-runs` 返回尝试记录。临时连接故障或服务端 5xx 最多共3次尝试（含首次）；限流、权限和参数错误立即暂缓。失败不写证据，不伪报接通。首批四个来源均首次成功。第二批国家统计局文章与 SEC 财务指标首次成功；工信部桌面文章 HTTP 403，改用官方移动站同文后成功。发现过程也已记录 source_runs。
