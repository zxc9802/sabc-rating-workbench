...
import { api, Detail, PilotPlan, Stage, Lifecycle } from '../lib/types';
...
type Props = { detail: Detail; busy: boolean; refresh: () => Promise<void>; run: (fn: () => Promise<void>, success?: string) => Promise<void>; full?: boolean; onFollowup?: (message: string) => void };

export function LifecyclePanel({ detail, busy, refresh, run, full, onFollowup }: Props) {
  ...
  if (!full) return <section className="stage-strip" aria-label="项目阶段">
    ...
    {!life.confirmed ? <p>请在“阶段评级报告与方案”中确认项目实际阶段，历史周期不会自动视为已完成。</p> : <p>{life.paused ? '已暂停回访' : due ? `${due.due ? '已到回访时间' : '下次回访'}：${due.date}` : '根据实际进度安排下一次回访'}{life.actual_start && ` · 实际开始 ${life.actual_start}`}{life.expected_end && life.stage === 'during' && ` · 预计结束 ${life.expected_end}`}</p>}
