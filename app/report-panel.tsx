...
const stageLabels = { pre: '启动前', during: '试点中', post: '试点后', legacy: '未标注阶段' };
const reportStage = (a: Assessment) => a.result.stage || a.snapshot.project.lifecycle?.stage || 'legacy';
...
export function ReportPanel({ detail, dimensions, busy, run, refresh }: { detail: Detail; dimensions: Dimension[]; busy: boolean; run: Run; refresh: () => Promise<void> }) {
  ...
  const currentStage = detail.project.lifecycle?.stage || 'pre';
  const [stage, setStage] = useState<Stage | 'legacy'>(currentStage);
  const reports = detail.assessments.filter(a => reportStage(a) === stage);
  const [selected, setSelected] = useState<Assessment | null>(reports[0] || null);
  const [confirmed, setConfirmed] = useState(false);
  const [edit, setEdit] = useState(false);
  useEffect(() => { setConfirmed(false); }, [proposal]);
  useEffect(() => { setStage(currentStage); setEdit(false); }, [detail.project.id, currentStage]);
  useEffect(() => { setSelected(detail.assessments.find(a => reportStage(a) === stage) || null); }, [detail.assessments, stage]);
  ...
  async function evaluate(manual = false) {
    await run(async () => { const record = await api<Assessment>('/projects/' + detail.project.id + '/assess', 'POST', manual ? { proposal, confirmed } : {}); setStage(currentStage); setSelected(record); await refresh(); if (record.result.grade !== 'NR') setEdit(false); }, '本次评估已保存，历史结果不会被覆盖');
  }
  ...
  return <section className="report-area"><div className="section-heading"><div><h2>SABC 阶段评级报告</h2><p>启动前、试点中、试点后独立留存，评级仅反映当时的信息与条件。</p></div><div className="button-group">{selected && <><button className="secondary" onClick={() => window.print()}><Printer size={15} />打印</button><a className="secondary" href={'/api/assessments/' + selected.id + '/export'}><Download size={15} />导出快照</a></>}<button className="primary" disabled={busy} onClick={() => { setStage(currentStage); if (detail.project.proposal) setEdit(true); else evaluate(); }}><RefreshCw size={15} />{'评估当前阶段 · ' + stageLabels[currentStage]}</button></div></div>
    {pendingFacts && <p className="review-note" role="status">模型整理了新的项目事实。请先到“项目资料”核对并保存，再生成本次评级。</p>}
    <div className="project-tabs" role="group" aria-label="报告阶段">{(Object.keys(stageLabels) as (Stage | 'legacy')[]).filter(key => key !== 'legacy' || detail.assessments.some(a => reportStage(a) === 'legacy')).map(key => {
      const latest = detail.assessments.find(a => reportStage(a) === key);
