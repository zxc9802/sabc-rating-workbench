...
export function SourceFetch({ projectId, busy, refresh, run }: { projectId: string; busy: boolean; refresh: () => Promise<void>; run: (task: () => Promise<void>, message?: string) => Promise<void> }) {
  ...
  useEffect(() => { api<NonNullable<typeof local>>('/local-sources').then(setLocal).catch(() => {}); }, []);
  return <details className="evidence-item"><summary>从官方接口或地方数据获取外部证据</summary><form className="form-section" onSubmit={e => {
    e.preventDefault(); run(async () => { try { await api(`/projects/${projectId}/sources/${source}`, 'POST', { query }); } catch { return; } await refresh(); });
    ...
    {source==='local' && <div className="form-section"><h3>各地区接入方式</h3>{local?.regions.map(region=><div className="evidence-item" key={region.id}><strong><a href={region.url} target="_blank" rel="noreferrer">{region.name}</a> · {region.method}</strong><p className="footnote">{region.note}</p>{region.example && <button type="button" disabled={busy} onClick={()=>setQuery(region.example!)}>填入已验证目录</button>}</div>)}
