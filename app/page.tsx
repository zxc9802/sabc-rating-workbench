...
import { api, waitForJob, Bootstrap, Detail, Project, RecordData, Settings, displayDate, text, gradeLabel } from '../lib/types';
...
import { ChatAttachments, uploadAttachment } from './chat-attachments';
...
const SCORE_CONFIRMATION = '请根据本轮提供的信息生成报告，给出评价结论和下一步建议。';

type ChatResult = { needs_collection?: boolean; report_id?: string; questions?: string[] };
...
function Workbench({ logout }: { logout: ReactNode }) {
  ...
  useEffect(() => { chatEnd.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, [tab, detail?.project.messages.length, pendingMessage, busy, streamReply]);
    ...
    if (fresh.active_jobs?.length) {
      try {
        for (const job of fresh.active_jobs) {
          receiveJob(job);
          await finishChat(id, await waitForJob<ChatResult>(job.id, receiveProgress, receiveJob));
        }
  ...
  function receiveJob(job: Job) {
    setCurrentJob(job);
    if (job.collection) setDetail(current => current && current.project.id === job.project_id ? { ...current, project: { ...current.project, lifecycle: job.collection!.lifecycle, report_ready: job.collection!.report_ready, report_pipeline: job.collection!.report_pipeline } } : current);
    if (job.generate_report || job.collection?.report_pipeline) setGeneratingReport(true);
  }
  async function finishChat(id: string, response: ChatResult) {
    ...
    else if (response.needs_collection) { setTab('chat'); setNotice('请先完成八维信息整理，再生成报告。'); }
  ...
  async function createProject() {
    if (!description.trim() && !projectFiles.length) { setError('请描述项目，或上传项目资料。'); return; }
    await run(async () => {
      const files = projectFiles;
      const p = await api<Project>('/projects', 'POST', {
        name: name.trim() || description.trim().slice(0, 24) || files[0].name.replace(/\.[^.]+$/, '').slice(0, 100),
        description: description.trim() || '请先阅读上传的项目资料，整理已知信息，再追问需要补充的内容。',
        auto_start: !files.length,
      });
      sessionStorage.setItem('sabc-project', p.id);
      setDetail({ project: p, evidence: [], assessments: [] }); setTab('chat'); setDescription(''); setName(''); setMessage(''); setProjectFiles([]);
      setStreamReply(''); setCurrentJob(null);
      const failures: string[] = [];
      try {
        for (const [index, file] of files.entries()) {
          try {
            await uploadAttachment(p.id, file, status => setUploadStatus(`资料 ${index + 1}/${files.length} · ${file.name}：${status}`));
          } catch (e) { failures.push(`${file.name}：${e instanceof Error ? e.message : '上传失败'}`); }
        }
      } finally { setUploadStatus(''); }
      if (files.length) await refreshProjectById(p.id);
      if (failures.length) {
        setError(`以下资料未能读取，请在当前项目中重新上传：${failures.join('；')}。已成功上传的资料已保留。`);
        return;
      }
      await reload();
  ...
  function receiveProgress(reply: string) {
    // Show the saved stage transition once, after the interview result is committed.
    const value = reply.trim();
    const closing = ['第一阶段问答已完成，正在整理报告。', '八维信息已梳理完成。', '八维收集完毕。'];
  ...
  async function sendMessage(content = message, generateReport = false, revision?: { revision_of: string; revision_turn_id: string }) {
    ...
    await run(async () => {
      ...
      try {
        ...
        if (!generateReport) setMessage(draft => draft || sent);
        throw error;
      } finally {
  ...
  async function saveProjectFacts(body: RecordData) {
    ...
    await run(async () => {
      ...
      await refreshProjectById(id); setTab('chat');
    });
  }
  async function confirmScoring() {
    await sendMessage(SCORE_CONFIRMATION, true);
  }
  ...
  const collection = collectionProgress(detail?.project.lifecycle, detail?.project.interview?.questions, detail?.project.report_ready === true || detail?.project.report_pipeline?.step === 'complete');
  const messages = detail?.project.messages || [];
  const pipeline = detail?.project.report_pipeline;
  const reviewing = pipeline?.step === 'reviewing' || pipeline?.step === 'revising';
  const pipelinePending = !!pipeline && pipeline.step !== 'complete';
  const pipelineActive = pipelinePending && busy && generatingReport;
  const processingLabel = pipeline?.step === 'complete' ? '最终报告已完成，正在打开…'
    : pipeline?.step === 'revising' ? '第二阶段：审查修订中…'
    : reviewing ? '第二阶段：报告审查中…' : '第一阶段问答已完成，正在整理报告…';
  ...
  return <div className="workspace">
    ...
    <div className="main-shell">
      ...
      {currentJob?.status === 'running' && <div className="feedback" role="status"><LoaderCircle size={18} className="spin" /><span>{generatingReport ? processingLabel : currentJob.phase === 'queued' ? '任务已提交，正在排队…' : streamReply ? '正在回答…' : '任务已提交，正在分析…'}</span><button className="secondary" onClick={stopReply}>停止回答</button></div>}
