...
function Workbench({ logout }: { logout: ReactNode }) {
  ...
  {scoringRequested && detail.project.lifecycle?.review ? <><strong>本轮阶段评分建议已生成</strong><p>{detail.project.lifecycle.review.summary}</p><button className="primary" onClick={() => setTab('report')}>核对并保存阶段评级报告</button><p>如有补充，可继续在下方发送，我们会重新核对。</p></> : <><strong>评分信息已收集完整</strong><p>八个维度已完成本阶段梳理。现在可以评分，还是有其他信息需要补充？</p>{data.dimensions.some(d => coverage?.[d.key]?.status !== 'known') && <p>尚未知、需外部核查或等待试点验证的内容会保留，不会当作已验证事实。</p>}<div className="scoring-actions"><button className="primary" disabled={attachmentBusy} onClick={confirmScoring}>开始评分</button><button className="secondary" onClick={() => document.getElementById('message')?.focus()}>我还有信息要补充</button></div><p>补充后会重新核对，再请你确认是否评分。</p></>}
          ...
          <><LifecyclePanel full detail={detail} dimensions={data.dimensions} busy={busy} run={run} refresh={refreshProject} />{detail.project.lifecycle?.confirmed && <ReportPanel detail={detail} dimensions={data.dimensions} busy={busy} run={run} refresh={refreshProject} />}</>}
        </main>}
