...
function Workbench({ logout }: { logout: ReactNode }) {
  ...
  async function createProject() {
    ...
    await run(async () => {
      const p = await api<Project>('/projects', 'POST', { name: name.trim() || description.trim().slice(0, 24), description, auto_start: true });
