...
function Workbench({ logout }: { logout: ReactNode }) {
  ...
  async function openProject(id: string) { await run(async () => {
    ...
    setReportError(''); setContinueSupplementing(false);
  ...
  async function createProject() {
    ...
    await run(async () => {
      const p = await api<Project>('/projects', 'POST', { name: name.trim() || description.trim().slice(0, 24), description, project_type: kind, auto_start: true });
  ...
  async function sendMessage(content = message, generateReport = false) {
    ...
    await run(async () => {
      setReportError(''); setContinueSupplementing(false); setGeneratingReport(generateReport); setPendingMessage(generateReport ? '' : sent); setMessage(''); setStreamReply(''); setCurrentJob(null);
  ...
  const collection = generatingReport ? { ready: true, complete: 8, percent: 100 } : collectionProgress(detail?.project.lifecycle, detail?.project.interview?.questions);
  const messages = detail?.project.messages || [];
  const informationReady = detail?.project.report_ready === true && !continueSupplementing;
  const hideClosingReply = informationReady && messages.at(-1)?.role === 'assistant';
  ...
  return <div className="workspace">
    ...
    <div className="main-shell">
      ...
      {currentJob?.status === 'running' && <div className="feedback" role="status"><LoaderCircle size={18} className="spin" /><span>{generatingReport ? '正在生成报告…' : currentJob.phase === 'queued' ? '任务已提交，正在排队…' : streamReply ? '正在回答…' : '任务已提交，正在分析…'}</span><button className="secondary" onClick={stopReply}>停止回答</button></div>}
      ...
      {!data ? <main className="loading-page"><LoaderCircle className="spin" /> <p>{error ? '应用服务尚未连接' : '正在打开工作台…'}</p><button className="secondary" onClick={() => run(async () => { await reload(); })}>重新连接</button></main> :
        ...
        !detail ? <main className="page-content projects-home">
          ...
          <>{<ReportPanel detail={detail} dimensions={data.dimensions} busy={busy} run={run} refresh={refreshProject} onGenerate={() => { if (detail.project.report_ready) confirmScoring(); else { setTab('chat'); setNotice('请先完成八维信息整理，再生成报告。'); } }} />}</>}
        </main>}
