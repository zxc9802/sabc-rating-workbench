...
import { interviewStage, interviewStages } from '../lib/interview-history';
...
function Workbench({ logout }: { logout: ReactNode }) {
  ...
  const [viewedStage, setViewedStage] = useState<Stage | 'legacy'>('pre');
  useEffect(() => { setViewedStage(detail?.project.lifecycle?.stage || 'pre'); }, [detail?.project.id, detail?.project.lifecycle?.stage]);
  ...
  useEffect(() => { chatEnd.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, [tab, viewedStage, detail?.project.messages.length, pendingMessage, busy, streamReply]);
  ...
  async function sendMessage(content = message, generateReport = false, advanceStage = false) {
    if (!content.trim() || !detail || voiceBusy || attachmentBusy || running.current) return;
    setTab('chat');
    setViewedStage(detail.project.lifecycle?.stage || 'pre');
    ...
    await run(async () => {
      ...
      try {
        if (advanceStage) {
          await api('/projects/' + detail.project.id + '/lifecycle', 'POST', { action: 'advance', payload: {}, version: detail.project.version });
          setDetail(await api<Detail>('/projects/' + detail.project.id));
        }
        await api('/projects/' + detail.project.id + '/chat', 'POST', { message: sent, generate_report: generateReport, field: last?.role === 'assistant' ? last.field : undefined }, setStreamReply, setCurrentJob);
      } catch (error) {
  ...
  const viewingCurrent = viewedStage === (detail?.project.lifecycle?.stage || 'pre');
  const stageMessages = detail?.project.messages.filter(m => interviewStage(m, detail) === viewedStage) || [];
  const informationReady = collection.ready && viewingCurrent;
  ...
  {scoringRequested && detail.project.lifecycle?.review ? <><strong>本轮阶段评分建议已生成</strong><p>{detail.project.lifecycle.review.summary}</p><button className="primary" onClick={() => setTab('report')}>核对并保存阶段评级报告</button><p>如有补充，可继续在下方发送，我们会重新核对。</p></> : <><strong>评分信息已收集完整</strong><p>八个维度已完成本阶段梳理。是否生成本阶段报告，还是有其他信息需要补充？</p>{data.dimensions.some(d => coverage?.[d.key]?.status !== 'known') && <p>尚未知、需外部核查或等待试点验证的内容会保留，不会当作已验证事实。</p>}<div className="scoring-actions"><button className="primary" disabled={attachmentBusy} onClick={confirmScoring}>生成报告</button><button className="secondary" onClick={() => document.getElementById('message')?.focus()}>我还有信息要补充</button></div><p>补充后会重新核对，再请你确认是否生成报告。</p></>}
          ...
          <>{detail.project.lifecycle?.confirmed && <ReportPanel detail={detail} dimensions={data.dimensions} busy={busy} run={run} refresh={refreshProject} />}</>}
        </main>}
