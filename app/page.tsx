...
function Workbench() {
  ...
  useEffect(() => { chatEnd.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, [detail?.project.messages.length, streamReply]);
  ...
  async function sendMessage() {
    if (!message.trim() || !detail || attachmentBusy) return;
    setStreamReply('');
    const last = detail.project.messages.at(-1);
    await run(async () => {
      await api('/projects/' + detail.project.id + '/chat', 'POST', { message, field: last?.role === 'assistant' ? last.field : undefined }, setStreamReply);
