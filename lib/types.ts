...
export type Score = { missing_evidence?: string; score: number | null; reason: string; basis: string; evidence_ids: string[]; support?: { quote: string; subject: string; metric: string; period: string; unit: string; use: 'support' | 'background' }[] };
...
export type Proposal = { decision_brief?: Record<string, string>; strongest_objections?: string[]; assessment_scope?: { subject: string }; dimensions: Record<string, Score>; assumptions: Assumption[]; pros: string[]; cons: string[]; policy_caps: string[]; vetoes: { reason: string; confirmed: boolean; evidence_ids: string[] }[]; s_conditions: Record<string, boolean> };
...
export type AssessmentReview = { time: string; notes: { perspective: 'value' | 'execution' | 'risk'; finding: string; evidence_ids: string[] }[]; questions: string[] };
export type ReportPipeline = { step: 'preparing' | 'reviewing' | 'revising' | 'complete'; report_id?: string | null };
export type Project = { report_pipeline?: ReportPipeline | null; interview?: { state: string; questions: string[] }; report_ready?: boolean; analysis_complete?: boolean; assessment_review?: AssessmentReview; id: string; name: string; description?: string; project_type: string | null; version: number; messages: Message[]; last_grade?: string; updated_at: string; proposal?: Proposal; pending_patch?: RecordData; lifecycle?: Lifecycle; followup?: Followup | null; [key: string]: unknown };
...
export type Rating = { revision?: { source_report_id: string; changes: string[] }; upgrade_requirements?: Record<string, string>; evidence_evaluated?: boolean; evidence_count?: number; unique_sources?: number; publisher_count?: number | null; deferral_reason?: string; stage?: Stage; provisional?: boolean; grade: string; status: string; base_score: number | null; base_grade: string | null; evidence_level: string; confidence: string; dimensions: (Score & Dimension & { weighted: number | null })[]; assumptions: Assumption[]; missing: string[]; triggered_rules: string[]; warnings: string[]; action: string; pros: string[]; cons: string[]; rule_version: string; resource_plan: { available_limit?: number; proposed_budget?: number | null; note?: string; formula?: string }; validation_plan: { claim: string; pass: string; fail: string; method: string }[]; reassessment_triggers: string[] };
...
export type Job = { collection?: { lifecycle: Lifecycle; report_ready: boolean; report_pipeline?: ReportPipeline | null }; id: string; generate_report?: boolean; project_id?: string; status: 'running' | 'success' | 'failed' | 'cancelled'; phase?: 'queued' | 'working'; result?: unknown; error?: string; partial_reply?: string };
...
// Legacy records retain their stored code; the UI presents a status, not a fifth grade.
export function gradeLabel(grade?: string, empty = '待评估') { return grade === 'NR' ? '暂缓评级' : grade || empty; }
