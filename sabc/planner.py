...
最多2条查询；没有相关的可用来源时返回空数组并说明能力缺口。禁止为了凑证据选择无关来源。
总reason最多500字，每条查询的reason最多300字，保持简洁。
...
地区统计期、抽样限制和更新时刻不能混淆。确需数据但缺少地区/标识时说明需要补充，不擅自猜测。'''
    ...
    system+='\n区分输入缺口与来源能力缺口：地区或标识未知时，只说明本轮尚无法匹配来源，不把它说成所有来源都不支持。只有能力清单明确证实才可断言某来源没有某类数据；不能把人口、GDP、零售额等示例概括为所有来源仅有总量。说明不取数的具体原因即可，不必列举未选择的地区或推断其字段。'
